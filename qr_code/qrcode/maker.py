...
def make_qr(data: Any, qr_code_options: QRCodeOptions):
    ...
    if isinstance(data, (bytes, int)):
        ...
        return segno.make(str(data), **qr_code_options.kw_make())


def make_qr_code_image(data: Any, qr_code_options: QRCodeOptions) -> bytes:
    """
    Returns a bytes object representing a QR code image for the provided `data`.

    :param str data: The data to encode
    :param qr_code_options: Options to create and serialize the QR code.
    :rtype: bytes
    """
    qr = make_qr(data, qr_code_options)
    out = io.BytesIO()
    qr.save(out, **qr_code_options.kw_save())
    return out.getvalue()


def make_embedded_qr_code(data: Any, qr_code_options: QRCodeOptions) -> str:
    """
...
def make_qr_code_with_args(data: Any, qr_code_args: dict) -> str:
    options = _options_from_args(qr_code_args)
    return make_embedded_qr_code(data, options)


def make_qr_code_url_with_args(data: Any, qr_code_args: dict) -> str:
    ...
    options = _options_from_args(qr_code_args)
    return make_qr_code_url(data, options, cache_enabled=cache_enabled,
